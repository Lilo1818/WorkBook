 ...
 * <p/>
 * The activity must use a theme that with translucent style. <style
 * name="Theme.Swipe.Back" parent="AppTheme"> <item
 * name="android:windowIsTranslucent">true</item> <item
 * name="android:windowBackground">@android:color/transparent</item> </style>
 * <p/>
...
public class SwipeBackLayout extends ViewGroup {
	...
	private DragEdge dragEdge = DragEdge.LEFT;// 默认从左往右滑动
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (getChildCount() > 0) {
			int measureWidth = MeasureSpec
					.makeMeasureSpec(getMeasuredWidth() - getPaddingLeft()
							- getPaddingRight(), MeasureSpec.EXACTLY);
			int measureHeight = MeasureSpec
					.makeMeasureSpec(getMeasuredHeight() - getPaddingTop()
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		case TOP:
		case BOTTOM:
			finishAnchor = finishAnchor > 0 ? finishAnchor : verticalDragRange
					* BACK_FACTOR;
			break;
		case LEFT:
		case RIGHT:
			finishAnchor = finishAnchor > 0 ? finishAnchor
					: horizontalDragRange * BACK_FACTOR;
			break;
		case NONE:
			break;
		}
	...
	private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
		...
		public int clampViewPositionVertical(View child, int top, int dy) {
			...
			} else if (dragEdge == DragEdge.BOTTOM && !canChildScrollDown()
					&& top < 0) {
		...
		public int clampViewPositionHorizontal(View child, int left, int dx) {
			...
			} else if (dragEdge == DragEdge.RIGHT && !canChildScrollLeft()
					&& left < 0) {
		...
		public void onViewPositionChanged(View changedView, int left, int top,
			...
			float fractionScreen = (float) draggingOffset
					/ (float) getDragRange();
			if (fractionScreen >= 1)
				fractionScreen = 1;

			if (swipeBackListener != null) {
				swipeBackListener.onViewPositionChanged(fractionAnchor,
						fractionScreen);
			}
		}
	...
	private boolean backBySpeed(float xvel, float yvel) {
		...
		case BOTTOM:
			if (Math.abs(yvel) > Math.abs(xvel)
				...
				return dragEdge == DragEdge.TOP ? !canChildScrollUp()
						: !canChildScrollDown();
			}
			break;
		case LEFT:
		case RIGHT:
			if (Math.abs(xvel) > Math.abs(yvel)
				...
				return dragEdge == DragEdge.LEFT ? !canChildScrollLeft()
						: !canChildScrollRight();
			}
			break;
		default:
