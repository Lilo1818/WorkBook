...
public class FollowAdapter extends MsBaseAdapter {

	private Handler handler;
	private final int CANCLE_FOLLOW = 1;//取消关注
	private final int OK_FOLLOW = 2;//关注
	
	public FollowAdapter(Context context, List<Object> modelList,
	...
	public void bindEvent(final int position, View convertView, ViewGroup parent) {
		ViewHolder viewHolder = (ViewHolder) convertView.getTag();
		final FollowModel followModel = (FollowModel) modelList.get(position);
		...
		viewHolder.icon.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				params.put(Constant.IntentKey.userID, followModel.UserId);
