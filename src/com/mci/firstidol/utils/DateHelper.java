...
public class DateHelper {
	...
	public enum DateStyle {
		...
		MM_DD("MM-dd"), YYYY_MM("yyyy-MM"), YYYY_MM_DD("yyyy-MM-dd"), MM_DD_HH_MM(
				...
				"yyyy-MM-dd HH:mm"), YYYY_MM_DD_HH_MM_SS("yyyy-MM-dd HH:mm:ss"), YYYY_MM_DD_HH_MM_SS_SSS(
	 ...
	 * @return
	 */
	public static List<String> getDayListOfWeek(String date) {
		...
		try {
			...
			calendar.add(Calendar.DAY_OF_WEEK, min - current); // 当天-基准，获取周开始日期
			Date start = calendar.getTime();
			dateStrings.add(sdf.format(start).split("-")[2]);
			for (int i = 1; i < 7; i++) {
				calendar.add(Calendar.DAY_OF_WEEK, 1); // 从第一天(周日)开始累加到周六
				Date next = calendar.getTime();
				dateStrings.add(sdf.format(next).split("-")[2]);
			}
		...
		// Log.i("xxxxxxxxxxxxxxxxxxxxxxx", dateStrings.toString());
		return dateStrings;
	}

	@SuppressLint("SimpleDateFormat")
	public static String intervalSinceNow(String theDate) {
		String dayResult = "";
		Date currentDate = new Date();
		Long currentTitme = currentDate.getTime();

		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		Date cDate;
		try {
			cDate = sdf.parse(theDate);
			long cTime = cDate.getTime();

			Long rTime = (currentTitme - cTime) / 1000;

			if (rTime / 60 < 1) {// 小于一分钟
				dayResult = rTime + "秒前";
			} else if (rTime / 3600 < 1) {// 小于一小时
				dayResult = rTime / 60 + "分钟前";
			} else if (rTime / 3600 >= 1 && rTime / 86400 < 1) {// 小于一天24小时
				dayResult = rTime / 3600 + "小时前";
			} else if (rTime / 86400 >= 1 && rTime / 86400 / 8 < 1) {// 大于一天而小于8天的
				dayResult = rTime / 86400 + "天前";
			} else {
				sdf = new SimpleDateFormat("yyyy-MM-dd");
				dayResult = sdf.format(cDate);
			}
	...
	/**
	 * 获取当前时间 。默认yyyy-MM-dd HH:mm:ss.sss格式。失败返回null。
	 * 
	 * @param date
	 *            日期字符串
	 * @return 日期
	...
	public static String getCurrentDate(String parttern) {
		...
		if (parttern != null && parttern.length() > 0) {
			...
			return getDateFormat(DateStyle.YYYY_MM_DD_HH_MM_SS_SSS.getValue())
					.format(date);
		}
	}
	...
	public static int getIntervalDays(Date date, Date otherDate) {
		...
		long time = Math.abs((date==null?0:date.getTime()) - otherDate.getTime());
	 ...
	 * 从时间戳字符串获取到毫秒数
	 */
	public static String getTimeFromDateStr(String dateStr){
		return dateStr.replace("/Date(", "").replace(")/", "");
	}
	
	
	public static String getStringFromDateStr(String dateStr,String parttern){
		if (TextUtils.isEmpty(dateStr)) {
			return "";
		}
		if (TextUtils.isEmpty(parttern)) {
			parttern = "yyyy-MM-dd HH:mm:ss";
		}
		SimpleDateFormat sdf =  new SimpleDateFormat(parttern);
		sdf.setTimeZone(TimeZone.getTimeZone("GMT+0"));
//		long time = Integer.parseInt(getTimeFromDateStr(dateStr));
		long time = Long.parseLong(getTimeFromDateStr(dateStr));
	 ...
	 * @return
	 */
	public static String getIntervalDays(String dateStr) {
		dateStr = getStringFromDateStr(dateStr,null);
		
		Date date = StringToDate(dateStr);
		
		Date currentDate = new Date();//当前日期
		long time = Math.abs((date==null?0:currentDate.getTime()) - date.getTime())/1000;
		
		long years  = (time)/(3600*24*30*384);
		long month  = (time)/(3600*24*30);
		long days   = (time)/(3600*24);
		long hours  = (time)%(3600*24)/3600;
		long minute = (time)%(3600*24)/60;
		String timeStr;
        if (years > 0) {
        	timeStr = years+"年";
        }else if (month > 0) {
